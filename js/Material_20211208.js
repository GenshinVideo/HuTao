...
function Save() {
  ...
  var CharacterSave = [...document.getElementsByClassName("noUiSlider")].map(Character=>{
    var [Min,Max] = Character.noUiSlider.get();
    localStorage.setItem(`${Character.id}` + "_Min", `${Min}`);
    return localStorage.setItem(`${Character.id}` + "_Max", `${Max}`);
  });
  var InventorySave = [...document.getElementsByClassName("validate")].map(Inventory=>{
    var Value = document.getElementById(`${Inventory.id}`).value;
    if("undefined" === `${Value}`) {var Value = "0"};
    localStorage.setItem(`${Inventory.id}`, `${Value}`);
  });
}
